...
// the kit; what this tool adds is the part only it knows: the version of the
// backend it drives, and how many WireGuard interfaces this host runs.
//
...
func runReport(cfg config.Config, opts options, out io.Writer) error {
	...
	if opts.demo {
		...
		info.Extra = append(info.Extra,
			report.Field{Key: "demo backend", Value: "wireguard"})
	} else {
	...
	if note := legacyConfigNote(cfg); note != "" {
		info.Extra = append(info.Extra,
			report.Field{Key: "config", Value: scrubHome(note)})
	}
...
// searchWG mirrors the backend's search paths, so the report finds a binary the
// tool would find.
var searchWG = []string{"/usr/bin/wg", "/bin/wg"}
